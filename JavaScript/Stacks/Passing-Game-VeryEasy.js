...
// 1. If element is 0, pop from stack, otherwise push the element to stack.
// 2. Return the stack top.

var passingGame = (A, B, C) => {
    const stack = [];
    for (let val of C) {
        if (val === 0) {
            if (stack.length)
    ...
    return stack.length ? stack[stack.length - 1] : B;
}

console.log(passingGame(10, 23, [86, 63, 60, 0, 47, 0, 99, 9, 0, 0]));
console.log(passingGame(1, 1, [2]));